...
function move_square(type) {
    let row = active_cell.parent().index();
    let column = active_cell.index();
    ...
    if (new_row < 0 || new_row >= m || new_column < 0 || new_column >= n) {
        ...
        let td1=table.find('tr').eq(row).find('td').eq(column);
        let td2=table.find('tr').eq(new_row).find('td').eq(new_column);
        aux = td1.html();
        td1.html(td2.html());
        td2.html(aux);
        active_cell = td2;
